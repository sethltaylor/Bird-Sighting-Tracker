...
st.set_page_config(
    layout="wide",
    page_title="Bird Sightings in DC, Maryland, and Virginia",
    page_icon=":bird:",
    menu_items={
...
def query_bird_sightings(common_name_selection):
    ...
    response = table.query(
        ...
        ProjectionExpression="howMany, locName, obsDt, sciName, lat, lng",
        KeyConditionExpression="comName = :common_name",
        ExpressionAttributeValues={":common_name": common_name_selection},
    )
...
with st.sidebar:
    ...
    common_name_selection = st.selectbox(
        "Select a bird", options=common_names["comName"].unique()
    )

    start_date_selection = st.date_input(
        "Start Date for Sightings Search",
...
with tab1:
    ...
    if st.button("Show Sightings"):
        df = query_bird_sightings(common_name_selection)
        ...
        if not df.empty:
            ...
            filtered_df = df[
                (df["obsDt"] >= start_datetime) & (df["obsDt"] <= end_datetime)
            ]
            ...
            data_dict = filtered_df[["lat", "lng", "locName", "howMany"]].to_dict(
                orient="records"
            )
            ...
            st.write(
                f"The total number of {common_name_selection} sighted between {start_date_selection} and {end_date_selection} was {round(filtered_df['howMany'].sum())}."
            )
            ...
            info_df = filtered_df[["locName", "howMany", "obsDt"]].rename(
                columns={
            ...
            st.dataframe(
                info_df.sort_values(by="Observation Date", ascending=False),
                use_container_width=True,
            )
        else:
            st.write(
                f"No {common_name_selection} were reported between {start_date_selection} and {end_date_selection}."
            )

with tab2:
    st.title("About")
    st.write(
        "This app is developed by Seth Taylor as a project to practice data engineering skills. This app is backed by a pipeline costing of AWS Lambda, DynamoDB, and S3. The app is hosted with AWS's Elastic Container Service. A more detailed overview is included in the Github repository below."
    )
    st.markdown(
        """
